...
## Дизайн ML системы - ScientificPaper-IR v0.1.0 <!-- omit from toc -->
...
- Два датасета для:
    - Baseline - практически или вовсе нет train-а; датасет урезан до ~30k сэмплов; добавлены аугментационные данные (графики вставлены в рандомные места статьи).
...
Для подготовки пилота подребуются следующие мощности:
...
_В ходе разработки основной модели архитектура может измениться и первоначальным вычеслительным мощности для её содержания могут быть перерасчитаны с учетом дальнейшей доработке для запуска в рамках первоначальных ограничений._
